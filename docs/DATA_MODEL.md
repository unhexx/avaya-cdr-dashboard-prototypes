...
Canonical store: **PostgreSQL 16**. Recordings media catalog may be **read** from MariaDB or PostgreSQL (`RECORDINGS_SQL_URL`) and **copied as metadata** into `recording_meta`.

Identifiers: English. Comments in migrations: Russian.
...
CREATE TYPE pbx_kind AS ENUM ('cm', 'ipo', 'session_manager', 'sbce', 'other');
CREATE TYPE log_kind AS ENUM ('sip', 'e1', 'alarm', 'sat', 'other');
```

## cdr_records

Canonical CDR (carry-forward from v1.0, productionized).
...
`raw_hash` = SHA-256 of `raw_record` UTF-8. UCID is stored when present (CM expanded / IPO Unique Call ID) but is **not** the only unique key — many unformatted streams lack it.
...
CREATE TABLE health_snapshots (
  ...
  status        VARCHAR(16) NOT NULL, -- ok, degraded, down, unknown
...
CREATE TABLE alarms (
  ...
  severity      VARCHAR(16) NOT NULL, -- critical, major, minor, warning
  code          VARCHAR(32),
  resource      VARCHAR(64), -- trunk, ds1, sip-entity, cpu
...
CREATE TABLE dialplan_entries (
  id            BIGSERIAL PRIMARY KEY,
  pbx_node_id   INTEGER REFERENCES pbx_nodes(id),
  source        VARCHAR(32) NOT NULL, -- ars, dialplan_analysis, ipo_shortcode, ipo_ars
...
CREATE TABLE log_events (
  ...
  alarm_type    VARCHAR(16), -- LOS, RAI, AIS, SLIP, CRC, BPV
  message       TEXT NOT NULL,
  raw           TEXT
);
...
CREATE TABLE recording_meta (
  ...
  encryption_hint VARCHAR(64), -- ipo_r11, none, unknown
...
Audio bytes are **not** stored in PostgreSQL. They are read from `RECORDINGS_MEDIA_ROOT` or a path column in the sql-source at request time.

## JSON document (API)

See `docs/API.md`. CDR list items match the v1.0 JSON Schema required fields: `start_time`, `duration_seconds`, `direction`.
